...
module.exports = Model = function(db, options) {
  var add, awaitingGetSnapshot, docs, getOps, getOpsInternal, load, makeOpQueue, model, refreshReapingTimeout, tryWriteSnapshot;
  ...
  makeOpQueue = function(docName, doc) {
    return queue(function(opData, callback) {
      ...
      return getOps(docName, opData.v, doc.v, function(error, ops) {
        var oldOp, snapshot, writeOp, _i, _len, _ref;
        if (error) {
        ...
        if (ops.length > 0) {
          try {
            for (_i = 0, _len = ops.length; _i < _len; _i++) {
              oldOp = ops[_i];
              if (oldOp.meta.source && opData.dupIfSource && (_ref = oldOp.meta.source, __indexOf.call(opData.dupIfSource, _ref) >= 0)) {
        ...
        return writeOp(docName, opData, function(error) {
          var oldSnapshot, _ref1;
          if (error) {
            console.warn("Error writing ops to database: " + error);
            return callback(error);
          }
          if ((_ref1 = options.stats) != null) {
            if (typeof _ref1.writeOp === "function") {
              _ref1.writeOp();
            }
          }
  ...
  load = function(docName, callback) {
    var callbacks, _ref, _ref1;
    if (docs[docName]) {
      if ((_ref = options.stats) != null) {
        if (typeof _ref.cacheHit === "function") {
          _ref.cacheHit('getSnapshot');
        }
      }
    ...
    if ((_ref1 = options.stats) != null) {
      if (typeof _ref1.cacheMiss === "function") {
        _ref1.cacheMiss('getSnapshot');
      }
    }
  ...
  tryWriteSnapshot = function(docName, callback) {
    ...
    if ((_ref = options.stats) != null) {
      if (typeof _ref.writeSnapshot === "function") {
        _ref.writeSnapshot();
      }
    }
  ...
  this.getOps = getOps = function(docName, start, end, callback) {
    var base, ops, version, _ref, _ref1, _ref2, _ref3;
    if (!(start >= 0)) {
    ...
    ops = (_ref1 = docs[docName]) != null ? _ref1.ops : void 0;
    if (ops) {
      ...
      if (start >= base || db === null) {
        ...
        if ((_ref2 = options.stats) != null) {
          _ref2.cacheHit('getOps');
        }
        return callback(null, ops.slice(start - base, end - base));
      }
    }
    if ((_ref3 = options.stats) != null) {
      _ref3.cacheMiss('getOps');
    }
  ...
  this.applyMetaOp = function(docName, metaOpData, callback) {
    var path, value, _ref;
    _ref = metaOpData.meta, path = _ref.path, value = _ref.value;
    if (!isArray(path)) {
